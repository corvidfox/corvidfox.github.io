#' @file artist_server.R
#' @title Server Module for Genre Panel
#'
#' Implements the server-side logic for the "by Artist" module of the 
...
#'
#' @seealso \code{\link{artist_ui}}, \code{\link{form_where_clause}}, 
#'   \code{\link{get_artist_data}}, \code{\link{build_artist_kpis}},
...
#' generation of KPI boxes, a by artist bar plot, and data table with export 
#' functionality. Designed for use with \code{artist_ui()}.
#'
...
#'   (date_range, country, artist, artist, metric).
...
#' @export
artist_server <- function(id, con, filters, styles) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(where_clause_reactive(), {
      ...
      data_raw <- get_artist_data(con, where_clause)
      ...
      kpis <- build_artist_kpis(agg_df, metric_col)
      
      # Pre-compute and store transformed artist-based data + KPI summaries
      cached_artist(list(raw = data_raw, data_all = data_all, kpis = kpis))
    })
    ...
    # --- KPI Card Outputs ---
    # 1. Top 5 Artists by Metric
    output$artists_box <- renderUI({
      safe_kpi_card(
        ...
        body_fn = function() {
          ...
          list(
            build_kpi(
              ...
              value = list_items,
              tooltip = "Top artists by the selected metric."
            ),
            build_kpi(
              label = "Total Artists", 
              value = k$num_artists, 
              tooltip = "Number of artists with available data."
            )
          )
        }, 
        title = "Top Artists",
        icon = bsicons::bs_icon("trophy-fill"),
        tooltip = "Top 5 artists, by the chosen metric.",
        styles = styles()
      )
    })
    ...
    output$artist_plot <- plotly::renderPlotly({
      # Styles
      s <- styles()
      ...
      # Restrict to top 20 artists of the metric (for space)
      selected_artists <- df |> 
        dplyr::select(artist) |> 
        dplyr::distinct()
      
      top_n <- min(nrow(selected_artists), 20)
      ...
      df <- df |>
        dplyr::filter(artist %in% selected_artists)
      ...
      p <- suppressWarnings(ggplot2::ggplot(
        df, 
        ggplot2::aes(
          x = artist, y = .data[[y_var]], fill = year, text = hover_text
        )) +
          ...
          # Labels
          ggplot2::labs(
            title = glue::glue("{y_label} by Artist (Top {top_n})"),
            x = "Artist",
            y = y_label,
            fill = "Year"
          ) %>%
      ...
      plotly::ggplotly(p, tooltip = "text") %>%
        style_plotly(
          styles = s,
          axes_labs = list(x = "Artist", y = y_label, legend = "Year")
        ) %>%
    ...
    output$download_csv <- shiny::downloadHandler(
      filename = function() {
        paste0("chinook_artist_", Sys.Date(), ".csv")
      },
      content = function(file) {
        write.csv(artist_data()$raw, file, row.names = FALSE)
      }
    )
  })
}
